...
<form action="{{ route('semeru.stock.store') }}" method="POST" enctype="multipart/form-data" class="stock-form">
@csrf
<div class="stock-form-card">
    ...
    {{-- BRANCH (OTOMATIS MENGAMBIL DAN TERKUNCI) --}}
    <h4 class="form-section">Branch Location</h4>
    <div class="form-group">
        <label>Assigned Branch</label>
        {{-- Input text palsu yang diberi style disabled/readonly agar user tidak bisa mengubahnya --}}
        <input type="text" 
               value="{{ auth()->user()->branch->branch_name ?? 'Semeru Branch' }}" 
               readonly 
               style="background-color: #f5f5f5; color: #666; cursor: not-allowed; font-weight: bold; border: 1px solid #eee;">
    </div>

    <div class="form-action">
        <a href="{{ route('semeru.stock') }}" class="btn-cancel">Cancel</a>
